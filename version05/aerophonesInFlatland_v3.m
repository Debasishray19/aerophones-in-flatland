    ...
    for row_idx = 2:frameH-1
        for col_idx = 2: frameW-1
            ...
            % Inject the source to the Vx_next and Vy_next = excitationV(T)
             PV_Nplus1(row_idx, col_idx, 2) = PV_Nplus1(row_idx, col_idx, 2) + excitationV(T)*excitation_weight(counter,1)*maxVxSigmaPrimedt(row_idx-1, col_idx-1);
             PV_Nplus1(row_idx, col_idx, 3) = PV_Nplus1(row_idx, col_idx, 3) + excitationV(T)*excitation_weight(counter,2)*maxVySigmaPrimedt(row_idx-1, col_idx-1);
            ...
            vb_alpha = (vb_alpha.* are_we_not_excitations(counter,:))*z_invCurr(counter);
